...
def _build_star_graph(query_emb, support_embs, support_nbrs, k, device):
    dists, idxs = support_nbrs.kneighbors(query_emb.reshape(1, -1), n_neighbors=k + 1)
    idxs  = idxs[0][1:]
    dists = dists[0][1:]
    x_nodes    = np.vstack([query_emb, support_embs[idxs]])
